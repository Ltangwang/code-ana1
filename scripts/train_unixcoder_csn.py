...
本脚本独立运行，不由 evaluate_code_search.py 或其它评测入口调用；训完将 output-dir 配到
config 的 code_search / clone_detection 所用 model_path 后再跑评测即可。

数据：与评测一致，支持 HuggingFace CodeSearchNet 与 GraphCodeBERT 清洗 JSONL（见 scripts/csn_data.iter_csn_jsonl）。

默认路径（可被 --train-jsonl / --valid-jsonl 覆盖）：
  - 训练：优先 CodeSearchNet_Dataset/java/train.jsonl，否则回退到代码检索评测用 java 目录
  - 验证：同目录下 validation.jsonl 或 valid.jsonl；或设 --valid-split-ratio 从 train 随机划分（清洗版推荐）

Java 清洗版（GraphCodeBERT 过滤目录）且避免误用原始 HF java 目录时，请用：
...
用法示例：
  ...
  python scripts/train_unixcoder_csn.py \\
    ...
    --output-dir /root/autodl-tmp/models/unixcoder-csn-java

默认超参（epochs=5、lr=1e-5、grad_accum=4、label_smoothing=0.05、AMP 等）。
``--train-max-samples`` 默认 0 表示全量读取 train-jsonl。每次运行会在 output-dir 写入 csn_train_args.json。

并发说明：
  - 本脚本为单进程单 GPU；可用 --num-workers>0 做 DataLoader 多进程读数据（非多卡训练）。
  - 多语言/多实验并行：开多个进程，分别 CUDA_VISIBLE_DEVICES=0/1 与不同 --output-dir。
  - 单任务多卡 DDP 未内置，需自行改分布式或换框架。

Python 清洗版且默认输出到 unixcoder-csn-python（不覆盖 Java）请用：
  python scripts/train_unixcoder_csn_python.py

Go 清洗版且默认输出到 unixcoder-csn-go 请用：
  python scripts/train_unixcoder_csn_go.py

JavaScript 清洗版且默认输出到 unixcoder-csn-javascript 请用：
  python scripts/train_unixcoder_csn_javascript.py

PHP 清洗版且默认输出到 unixcoder-csn-php 请用：
  python scripts/train_unixcoder_csn_php.py

Ruby 清洗版且默认输出到 unixcoder-csn-ruby 请用：
